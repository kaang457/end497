...
--------------------------------------------------------------
Orijinal (hızlı) versiyon baz alınarak revize edildi.
Arkadaşının orijinal kodundaki "op_adaylar" (yetkinlik/aday) kontrolleri 
ve hız haritası (worker_speed_map) geri getirildi.
  - Darboğaz kısıtı: stage3_bottleneck kullanılıyor
  - No-Worsening kısıtı: stage3 MAD'i baz alıyor
  - Arc oluşturma: küçük operasyonlar (op_std <= 2.0) transfer dışı tutuldu
  - İç Dengeleme (ST4) ve GEZİCİ etiketleri korundu
  - Sabit istasyon (fixed_stations) koruması korundu
"""
...
def _worker_speed(engine, s):
    """Sadece ana istasyon işçisinin hızını döndürür."""
...
def _calc_total_mad(values):
    """Varyans yerine doğrudan TOPLAM MAD hesaplar"""
    if not values:
        return 0.0
...
def run(engine):
    ...
    # ---------------------------------------------------------------
    # İŞÇİ HIZ HARİTASI (Orijinal Doğru Mantık Geri Getirildi)
    # ---------------------------------------------------------------
    worker_speed_map = {}
    ...
    # ---------------------------------------------------------------
    # ARC (ROTA) OLUŞTURMA & YETKİNLİK KONTROLÜ
    # ---------------------------------------------------------------
    valid    = []
    seen_set = set()
    ...
    # K4: Kötüleşmeme Kısıtı (No-Worsening)
    ...
    # Amaç: Toplam Mutlak Sapmayı Minimize Et
    ...
    final_bn_w     = max(final_loads, key=final_loads.get) # Darboğazı yaratan işçi
    ...
    engine.log(f"       Taşınan İş Sayısı: {len(moved_ops_set)} op")
    engine.log("=" * 60)

    engine.print_stage_summary("STAGE 4")
    
    # Arayüz ve JSON çıktıları için gerekli istatistikler (Senin eklemelerin)
    engine.stage4_stats = {
