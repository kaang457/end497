...
def run(engine, pool):
    ...
    # --- STRATEJİK MERGE ARKLARI (stage2_a'dan eklendi) ---
    # Offset ile elle tarama YERİNE Stage 1'deki neighbor_map kullanılıyor.
    # Bu sayede merge kararı da CPLEX'e devrediliyor.
    merge_arcs = []
    ...
    m2.ARCS_MERGE = pyo.Set(initialize=merge_arcs,   dimen=2)  # stage2_a'dan eklendi

    m2.y_pool  = pyo.Var(m2.ARCS_POOL,  domain=pyo.Binary)
    m2.y_mast  = pyo.Var(m2.ARCS_MAST,  domain=pyo.Binary)
    m2.y_merge = pyo.Var(m2.ARCS_MERGE, domain=pyo.Binary)     # stage2_a'dan eklendi
    ...
    def obj_rule_2(model_obj):
        ...
        c_merge = sum(10000 * model_obj.y_merge[s, n]   for (s, n)   in model_obj.ARCS_MERGE)  # stage2_a'dan eklendi
    ...
    for s in engine.empty_stations:
        ...
        # 1. Pool kontrolü (hız çarpanı: 1.2 — stage2_a'dan güncellendi, eskisi 1.0)
        if s in s_to_p:
            for h in s_to_p[s]:
        ...
        # 2. Master kontrolü (hız çarpanı: 0.8)
        ...
        # 3. Merge kontrolü — Stage 1 komşu haritasına göre (stage2_a'dan eklendi)
    ...
    # Birleştirilen (kapanan) istasyonları sil
    for s in stations_to_delete:
        if s in engine.final_stations: del engine.final_stations[s]

    # Kalan havuz ve ustaları hesapla (stage2_a'dan: tek seferde tüm busy_workers çıkarılıyor)
